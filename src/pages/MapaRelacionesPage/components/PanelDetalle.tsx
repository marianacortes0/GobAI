...
export function PanelDetalle({ nodo, contexto, onClose }: Props) {
  ...
  const buscarNodo = (id: string) => contexto?.nodos.find((n) => n.id === id)
  ...
  return (
    <aside className="absolute right-0 top-0 z-30 flex h-full w-96 flex-col border-l border-slate-700 bg-slate-900/95 shadow-2xl backdrop-blur">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {nodo.nivel_riesgo && (
              ...
              >
                {nodo.nivel_riesgo === 'alto' && <AlertTriangle className="w-3 h-3" />}
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 text-sm text-slate-300 scrollbar-thin">
        ...
        {tab === 'Conexiones' && (
          <Conexiones
            conexiones={conexiones.map((c) => {
              ...
              return { tipo: c.tipo, nodo: buscarNodo(otroId), direccion: c.source === nodo.id ? 'out' : 'in' as const }
            })}
          />
        )}
      </div>
          ...
          <li key={a.id} className="rounded-lg border border-slate-700/60 bg-slate-800/40 p-3 text-xs">
            ...
            <p className="text-slate-400">
              {a.tipo === 'sancion' && a.metadata?.monto !== undefined
                ? `Monto: ${formatCurrency(a.metadata.monto)} · ${formatDate(a.metadata.fecha ?? '')}`
                : a.metadata?.fuente
...
function Conexiones({
  ...
  return (
    <>
      <p className="mb-2 text-xs text-slate-500">{formatNumber(conexiones.length)} conexiones</p>
